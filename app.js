...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
const REFERENCE_NOTICE = 'Dados de referência configuráveis — confirmar conforme edição normativa adotada. Não reproduzem tabela oficial da ABNT NBR 5410.';
...
const AMP_BASE = { 1.5: 15.5, 2.5: 21, 4: 28, 6: 36, 10: 50, 16: 68, 25: 89, 35: 110, 50: 134, 70: 171, 95: 207, 120: 239 };
...
const METHODS = {
  B1: { label: 'Eletroduto embutido em alvenaria/laje', factor: 1.00 },
...
const MATERIAL_FACTOR = { Cu: 1.00, Al: 0.78 };
const ISOLATION_FACTOR = { PVC70: 1.00, XLPE90: 1.11 };
...
const K1_TABLE = {
  PVC70: { 20: 1.08, 25: 1.04, 30: 1.00, 35: 0.96, 40: 0.91, 45: 0.87, 50: 0.82, 55: 0.76, 60: 0.71 },
  XLPE90: { 20: 1.06, 25: 1.03, 30: 1.00, 35: 0.97, 40: 0.94, 45: 0.90, 50: 0.87, 55: 0.83, 60: 0.79, 65: 0.74, 70: 0.70 }
};
...
const K2_TABLE = { 1: 1.00, 2: 0.80, 3: 0.70, 4: 0.65, 5: 0.60, 6: 0.57, 7: 0.54, 8: 0.52, 9: 0.50 };
function k2For(n) { if (n <= 1) return 1; if (K2_TABLE[n] !== undefined) return K2_TABLE[n]; return n >= 9 ? 0.45 : K2_TABLE[9]; }
...
function peFor(sf) { if (sf <= 16) return sf; if (sf <= 35) return 16; return sf / 2; }
...
const COND_DIAM = { 1.5: 3.0, 2.5: 3.4, 4: 3.9, 6: 4.4, 10: 5.6, 16: 6.6, 25: 8.2, 35: 9.4, 50: 11.5, 70: 13.2, 95: 15.6, 120: 17.4 };

const TYPES = ['Chuveiro', 'Forno elétrico', 'Máquina de lavar', 'Ar-condicionado', 'Torneira elétrica', 'Bomba', 'Cooktop', 'Micro-ondas', 'Lava-louças', 'Outro'];
const ROOM_TYPES = ['Quarto', 'Sala', 'Cozinha', 'Banheiro', 'Área de serviço', 'Corredor', 'Varanda', 'Garagem', 'Outro'];
...
function validateAll() {
  ...
  S.rooms.forEach(r => {
    ...
    if (r.l <= 0 || r.w <= 0) errs.push(`Cômodo "${r.name || '(sem nome)'}": comprimento e largura devem ser maiores que zero.`);
  });
  ...
  S.loads.forEach(l => {
    ...
    if (l.fp <= 0 || l.fp > 1) errs.push(`Equipamento "${l.name || l.type}": fator de potência deve estar entre 0 (exclusive) e 1.`);
  ...
  S.circuits.forEach(c => {
    ...
    if (S.installation.sistema === 'mono' && c.phase !== 'A-N') errs.push(`Circuito "${c.name}": sistema monofásico só admite A-N.`);
  });
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
function calcularArea(r) { return N(r.l) * N(r.w); }
function calcularPerimetro(r) { return 2 * (N(r.l) + N(r.w)); }
...
function fasesEnvolvidas(opt) { return opt.split('-').filter(x => x !== 'N'); }
function precisaNeutro(opt) { return opt.includes('N'); }
...
function fatorK1() {
  ...
  for (let i = 0; i < temps.length - 1; i++) {
    if (t >= temps[i] && t <= temps[i + 1]) {
      const t0 = temps[i], t1 = temps[i + 1], v0 = tab[t0], v1 = tab[t1];
      return t1 === t0 ? v0 : v0 + (v1 - v0) * (t - t0) / (t1 - t0);
    }
  }
  return tab[temps[temps.length - 1]];
}
function fatorK2() { return k2For(Math.max(1, Math.round(N(S.installation.agrupamento) || 1))); }
...
function capacidadeCorrigida(secao, k1, k2) { return capacidadeBase(secao) * k1 * k2; }
...
/* --------------------------------------------------------------------------
   9. DEMANDA
   -------------------------------------------------------------------------- */
function grupoDemanda(c) { return c.type === 'Iluminação' ? 'iluminacao' : c.type === 'TUG' ? 'tug' : 'tue'; }
...
function calcularQuedaTensao(c, secao, Ib) {
  ...
  return { formula: trifasico ? '√3·ρ·L·Ib·cosφ/S' : '2·ρ·L·Ib·cosφ/S', dropV, dropPct, limite, atende: dropPct <= limite };
}
...
function dimensionarPE(secaoFase) {
  ...
  return { secaoFase, secaoPE: spe, criterio: 'Redução por faixa de seção da fase (Sf≤16→Spe=Sf; 16<Sf≤35→Spe=16; Sf>35→Spe=Sf/2) — ' + REFERENCE_NOTICE };
}
function dimensionarNeutro(c, secaoFase) {
  if (!precisaNeutro(c.phase)) {
    return { aplicavel: false, secaoNeutro: null, status: 'NÃO APLICÁVEL', observacao: 'Circuito entre fases, sem condutor neutro.' };
  }
  if (c.phase === 'A-B-C') {
    return { aplicavel: true, secaoNeutro: secaoFase, status: 'NECESSITA VERIFICAÇÃO', observacao: 'Dimensionamento do neutro requer verificação complementar (equilíbrio de fases, presença de harmônicas e cargas monofásicas conectadas ao circuito trifásico).' };
  }
  // circuito fase-neutro monofásico: adota-se, como ponto de partida,
  // neutro de mesma seção da fase.
  return { aplicavel: true, secaoNeutro: secaoFase, status: 'NECESSITA VERIFICAÇÃO', observacao: 'Seção do neutro adotada igual à seção da fase como ponto de partida; confirmar conforme critério da edição normativa adotada.' };
}
...
function dimensionarEletroduto(c, secaoFase, secaoPE) {
  ...
  if (!Number.isFinite(diametroMinimo) || diametroMinimo <= 0) {
    return { status: 'NECESSITA VERIFICAÇÃO', observacao: 'Não foi possível realizar o dimensionamento completo do eletroduto.' };
  }
  return {
    qtdCondutores, areaCondutor, areaPE, areaTotal, ocupacao: ocupacao * 100,
    diametroMinimo, comercial: comercial ?? CONDUIT_SIZES[CONDUIT_SIZES.length - 1],
...
function calcularBalanceamento() {
  ...
  ordenado.forEach(c => {
    ...
    if (!escolhida) {
      let melhor = opts[0], melhorScore = Infinity;
      opts.forEach(opt => {
        ...
        if (score < melhorScore) { melhorScore = score; melhor = opt; }
      });
      escolhida = melhor;
    }
...
function verificarDR(c, roomType) {
  ...
  if (c.type === 'TUE' && roomType === 'Banheiro') {
    return { status: 'NECESSITA VERIFICAÇÃO', observacao: 'Circuito de equipamento em banheiro: tipicamente objeto de proteção diferencial (DR). Confirmar conforme esquema de aterramento e critérios aplicáveis.' };
  }
  if (areaMolhada || c.type === 'TUG') {
    return { status: 'NECESSITA VERIFICAÇÃO', observacao: 'Circuito de tomada em área que pode envolver umidade/contato com pessoas: avaliar necessidade de proteção diferencial (DR). Não há seleção automática universal.' };
  }
  return { status: 'NECESSITA VERIFICAÇÃO', observacao: 'Avaliar necessidade de DR conforme a função do circuito e as condições reais de instalação.' };
}
function verificarDPS() {
  return { status: 'NECESSITA VERIFICAÇÃO', observacao: 'Verificar necessidade e especificação de DPS conforme as características da instalação e critérios normativos aplicáveis. O software não realiza seleção normativa completa.' };
}
...
function calc(c) {
  ...
  return { Ib, V: tensaoCircuito(c), k1, k2, secao, IzBase, Iz, queda, disjuntor, pe, neutro, eletroduto, demanda, dr, status };
}
...
function diagramaSVG() {
  const w = 760, rowH = 34, top = 60, left = 210;
...
function renderRooms() {
  const el = $('rooms');
  if (!S.rooms.length) { el.innerHTML = '<div class="info-box">Nenhum cômodo cadastrado.</div>'; return; }
  el.innerHTML = S.rooms.map(r => `<div class="room-card"><div class="panel-head"><h3>${esc(r.name)}</h3><button class="danger" onclick="removeRoom('${r.id}')">Remover</button></div>
    <div class="form-grid">
    ...
    <p><b>Área:</b> ${fmt(calcularArea(r))} m² &nbsp; <b>Perímetro:</b> ${fmt(calcularPerimetro(r))} m</p></div>`).join('');
}
function renderLighting() {
  $('lighting').innerHTML = S.rooms.length ? `<div class="table-wrap"><table class="data-table"><tr><th>Ambiente</th><th>Área</th><th>Carga de dimensionamento</th><th>Potência nominal instalada (opcional)</th></tr>
    ...
    </table></div>` : '<div class="info-box">Cadastre os cômodos.</div>';
}
function renderTugs() {
  $('tugs').innerHTML = S.rooms.length ? S.rooms.map(r => {
    const min = calcularTUGMinimo(r), adotado = r.tugs ?? min;
  ...
  }).join('') : '<div class="info-box">Cadastre os cômodos.</div>';
}
function renderLoads() {
  $('loads').innerHTML = S.loads.length ? S.loads.map(l => `<div class="load-card"><div class="panel-head"><h3>${esc(l.name || l.type)}</h3><button class="danger" onclick="removeLoad('${l.id}')">Remover</button></div>
    <div class="form-grid">
    ...
    </div></div>`).join('') : '<div class="info-box">Nenhum equipamento específico.</div>';
}
...
function rebuildCircuits() {
  ...
  if (S.rooms.length) {
    arr.push({
      id: 'circ-iluminacao', name: 'C1 - Iluminação', type: 'Iluminação',
      p: S.rooms.reduce((t, r) => t + calcularIluminacao(calcularArea(r)), 0),
      d: N(S.installation.distAlim) || 10, fp: 1, tensaoTipo: 'fn', roomRef: null
    });
  }
  if (S.rooms.length) {
    arr.push({
      id: 'circ-tug', name: 'C2 - TUG', type: 'TUG',
  ...
  S.loads.forEach((l, i) => {
    arr.push({
      id: 'circ-load-' + l.id, name: `C${i + 3} - ${l.name || l.type}`, type: 'TUE',
      p: N(l.p) * N(l.q || 1), d: N(l.d || 10), fp: N(l.fp || 1),
      tensaoTipo: tensaoTipoDaCarga(N(l.v)), loadId: l.id, roomRef: l.room || null
    });
  });
  // preserva tipo de ligação manual (ex.: forçar trifásico) já escolhido antes
  arr.forEach(c => { const prev = S.circuits.find(p => p.id === c.id); if (prev && prev.tensaoTipoManual) { c.tensaoTipo = prev.tensaoTipo; c.tensaoTipoManual = true; } });
  S.circuits = arr;
  const bal = calcularBalanceamento();
  S.circuits.forEach(c => { c.phase = bal.assign[c.id]; });
  renderAll();
}
function tipoLigacaoChange(id, tipo) {
  const c = S.circuits.find(x => x.id === id); if (!c) return;
  c.tensaoTipo = tipo; c.tensaoTipoManual = true;
  const opts = opcoesFasePara(tipo);
  if (!opts.includes(c.phase)) { delete S.faseOverride[c.id]; c.phase = opts[0]; }
  renderAll();
}
...
function resetBalanceamento() { S.faseOverride = {}; const bal = calcularBalanceamento(); S.circuits.forEach(c => { c.phase = bal.assign[c.id]; }); renderAll(); }

function renderCircuits() {
  $('circuits').innerHTML = S.circuits.length ? S.circuits.map(c => {
    const opts = opcoesFasePara(c.tensaoTipo || 'fn');
    const ligacaoOpts = S.installation.sistema === 'mono' ? [['fn', 'Fase-neutro']]
      : S.installation.sistema === 'bi' ? [['fn', 'Fase-neutro'], ['ff', 'Fase-fase']]
      : [['fn', 'Fase-neutro'], ['ff', 'Fase-fase'], ['3f', 'Trifásico (A-B-C)']];
  ...
  }).join('') : '<div class="info-box">Clique em Recalcular.</div>';
}

function renderDimensioning() {
  if (!S.circuits.length) { $('dimensioning').innerHTML = '<div class="info-box">Crie os circuitos primeiro.</div>'; return; }
  const rows = S.circuits.map(c => {
    const x = calc(c); c.calc = x;
...
function renderQdc() {
  const bal = calcularBalanceamento();
  const rows = S.circuits.map(c => { const x = calc(c); return `<tr><td>${esc(c.name)}</td><td>${c.type}</td><td>${c.phase}</td><td>${fmt(c.p, 0)} VA</td><td>${x.secao} mm²</td><td>${x.disjuntor.In ?? '—'} A</td><td>${x.pe.secaoPE} mm²</td><td>${x.eletroduto.comercial ?? '—'} mm</td><td><span class="tag ${x.dr.status === 'ATENDE' ? 'ok' : 'warn'}">${x.dr.status}</span></td></tr>`; }).join('');
...
function renderAll() { renderAlerts(); renderRooms(); renderLighting(); renderTugs(); renderLoads(); renderCircuits(); renderDimensioning(); renderQdc(); }
...
function removeRoom(id) {
  ...
  if (associadas.length && !confirm(`Este cômodo possui ${associadas.length} equipamento(s) associado(s). Eles ficarão sem cômodo definido. Deseja continuar?`)) return;
  S.rooms = S.rooms.filter(x => x.id !== id);
  S.loads = S.loads.map(l => l.room === id ? { ...l, room: '' } : l);
  rebuildCircuits();
}
function removeLoad(id) { S.loads = S.loads.filter(x => x.id !== id); rebuildCircuits(); }

$('addRoom').onclick = () => { S.rooms.push({ id: uid(), name: `Cômodo ${S.rooms.length + 1}`, type: 'Quarto', l: 3, w: 3, tugVA: 100, lamp: 0 }); renderAll(); };
$('addLoad').onclick = () => { S.loads.push({ id: uid(), type: 'Chuveiro', name: '', room: '', p: 5500, v: S.installation.vf, fp: 1, q: 1, d: 10 }); renderAll(); };
$('rebuildCircuits').onclick = rebuildCircuits;

function bind() {
  ['tipo', 'sistema', 'vf', 'vff', 'freq', 'material', 'isolacao', 'metodo', 'temp', 'agrupamento', 'quedaLimite', 'distAlim', 'expansao', 'ocupacaoEletroduto'].forEach(id => {
    if (!$(id)) return;
    $(id).addEventListener('change', () => {
      const v = $(id).value;
      const numeric = ['vf', 'vff', 'freq', 'temp', 'agrupamento', 'quedaLimite', 'distAlim', 'expansao', 'ocupacaoEletroduto'];
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
function example() {
  Object.assign(S.installation, { tipo: 'residencial', sistema: 'bi', vf: 127, vff: 220, freq: 60, material: 'Cu', isolacao: 'PVC70', metodo: 'B1', temp: 30, agrupamento: 1, quedaLimite: 4, distAlim: 12, expansao: 20, ocupacaoEletroduto: 40 });
  const sala = uid(), quarto = uid(), cozinha = uid(), banheiro = uid();
  S.rooms = [
  ...
  S.loads = [
    ...
    { id: uid(), type: 'Forno elétrico', name: 'Forno', room: cozinha, p: 3000, v: 220, fp: .95, q: 1, d: 10 },
    { id: uid(), type: 'Ar-condicionado', name: 'Ar-condicionado', room: quarto, p: 1200, v: 127, fp: .9, q: 1, d: 15 }
  ];
  S.faseOverride = {};
  rebuildCircuits(); showStep(1); syncInputs();
}
function syncInputs() { Object.entries(S.installation).forEach(([k, v]) => { if ($(k)) $(k).value = v; }); }
$('btnExemplo').onclick = example;
$('btnReset').onclick = () => { if (confirm('Limpar projeto?')) location.reload(); };
...
const Persistence = {
  ...
  importJSON(text) { const x = JSON.parse(text); Object.assign(S, x); if (!S.faseOverride) S.faseOverride = {}; },
  saveLocal() { try { localStorage.setItem('dimensionaLar-projeto', this.exportJSON()); } catch (e) { /* localStorage indisponível — ignorar silenciosamente */ } },
  loadLocal() { try { const t = localStorage.getItem('dimensionaLar-projeto'); if (t) this.importJSON(t); } catch (e) { /* nada salvo ou indisponível */ } }
};
$('btnExportar').onclick = () => { const blob = new Blob([Persistence.exportJSON()], { type: 'application/json' }), a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'dimensionaLar-projeto.json'; a.click(); URL.revokeObjectURL(a.href); };
$('importarJson').onchange = e => {
  const f = e.target.files[0]; if (!f) return;
  const rd = new FileReader();
  rd.onload = () => { try { Persistence.importJSON(rd.result); syncInputs(); rebuildCircuits(); showStep(1); } catch { alert('JSON inválido.'); } };
  rd.readAsText(f);
};
...
function gerarRelatorioHTML() {
  ...
  const memorialRows = results.map(({ c, x }) => linhaCalc(
    c.name,
  ...
  ${S.rooms.map(r => { const min = calcularTUGMinimo(r), ad = r.tugs ?? min; return `<tr><td>${esc(r.name)}</td><td>${min}</td><td>${ad}</td><td>${r.tugVA ?? 100}</td><td>${fmt(ad * (r.tugVA ?? 100))} VA</td></tr>`; }).join('')}</table>
...
$('btnRelatorio').onclick = () => { const w = window.open(); w.document.write(gerarRelatorioHTML()); w.document.close(); w.print(); };
